...
#### 💡 배운 점
- `atomic`, `SRWLOCK`처럼 주소로 상태를 추적하는 객체는 이동하면 안 된다
- IOCP처럼 커널이 객체 주소를 보관하는 구조에서는 객체가 생명 주기 내내 같은 주소에 있어야 한다
- STL 컨테이너에 무거운 객체를 직접 담을 때는 재할당 시 동작을 미리 확인해야 한다
...
#### 💡 배운 점
- 상태를 바꾸면서 동시에 정리 작업도 해야 하는 경우, 락보다 CAS로 처리하는 게 깔끔할 때가 있다
- 종료 처리 같은 정리 로직은 한 곳에 모아두지 않으면 빠뜨리거나 중복 실행하기 쉽다
...
- 락이 어디서 잠기는지 함수 시그니처만으로는 추적이 어려움 → 유지보수성 저하
- 임시방편으로 `BroadCastUnsafe()` 같은 "락 안 잡는 내부용" 함수를 만들어야 했고, 호출자 책임으로 동기화를 떠넘기게 되는 상황이 발생
...
- 부자연스러웠던 `BroadCastUnsafe()` 같은 함수가 사라지고, `BroadCast()` 하나로 통합
...
#### 💡 배운 점
- 두 객체의 락이 항상 같은 순서로 잡힌다면, 굳이 따로 둘 필요 없이 하나로 합치는 게 낫다
- 락을 하위 객체에 분산시키면 호출 경로마다 신경 써야 할 게 늘어난다
